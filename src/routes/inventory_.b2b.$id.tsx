...
import { fetchB2bById, updateB2bPartner } from "@/lib/b2bService";
import type { B2bPartner } from "@/data/inventoryStore";

export const Route = createFileRoute("/inventory_/b2b/$id")({ component: EditB2bPage });
...
function EditB2bPage() {
  ...
  if (!partner) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        <p className="text-lg font-semibold text-muted-foreground">Partner not found</p>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      navigate({ to: "/inventory", search: { tab: "b2b" } });
    } catch (err) {
      console.error("[B2B Edit]", err);
